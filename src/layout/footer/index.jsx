import Button from "@/components/button";
import Container from "@/components/container";
import Paragraph from "@/components/paragraph";
import { pagesList, primaryColor } from "@/utils/consts";
import Link from "next/link";
import { FiPhone } from "react-icons/fi";
import { MdMailOutline } from "react-icons/md";

const Footer = () => {
  return (
    <footer className="py-[50px] bg-darkBlue text-white">
      <Container>
        <div className="grid place-items-center  md:text-start md:grid-cols-3 gap-10 md:gap-6">
          <div className="w-fit flex flex-col items-center md:items-start gap-4">
            <Link className="w-full" href={"/"}>
              <img
                src="data:image/png;base64,"
                className="w-full h-[120px] object-contain object-center"
                width={100}
                height={70}
              />
            </Link>
            <Link
              href={"tel:+998979101387"}
              className="flex items-center gap-2"
            >
              <FiPhone size={"20px"} color={primaryColor} />
              <span className="">+998 97 910 13 87</span>
            </Link>
            <Link
              href={"tel:+998972853355"}
              className="flex items-center gap-2"
            >
              <FiPhone size={"20px"} color={primaryColor} />
              <span className="">+998 97 285 33 55</span>
            </Link>
            <Link
              href={"tel:+998972884400"}
              className="flex items-center gap-2"
            >
              <FiPhone size={"20px"} color={primaryColor} />
              <span className="">+998 97 288 44 00</span>
            </Link>
            <Link
              href={"mailto:Info@Voyage-Avicenne.com"}
              className="flex items-center gap-2"
            >
              <MdMailOutline size={"20px"} color={primaryColor} />
              <span className="">Info@Voyage-Avicenne.com</span>
            </Link>
          </div>
          <div className="w-fit mx-auto">
            <Paragraph className="text-white !text-2xl text-center mb-6 !font-semibold">
              Quick links
            </Paragraph>
            <div className=" flex flex-col items-center gap-3 ">
              {pagesList?.map((item) => (
                <Link
                key={item?.id}
                  href={item?.url}
                  className="text-xl hover:text-primary transition-all duration-500"
                >
                  {item?.name}
                </Link>
              ))}
            </div>
          </div>
          <div className=" md:mx-0 md:max-w-[300px] w-[80%] mx-auto md:w-full md:ml-auto">
            <Paragraph className="text-white !text-2xl text-center mb-6 !font-semibold">
              Send message
            </Paragraph>
            <div className="flex flex-col gap-3 ">
                <input placeholder="Name" type="text" className="outline-none border-white border-2 w-full bg-transparent rounded-xl text-base p-2" />
                <input placeholder="Phone" type="text" className="outline-none border-white border-2 w-full bg-transparent rounded-xl text-base p-2" />
                <textarea rows={3} placeholder="Message" type="text" className="resize-none outline-none border-white border-2 w-full bg-transparent rounded-xl text-base p-2" />
                <Button className={"w-full text-center"} >Send</Button>
            </div>
          </div>
        </div>
      </Container>
    </footer>
  );
};

export default Footer;
